...
`manuscript/CARRIED_FORWARD.md`. Two gates run at the end of every execution:
`pipeline/verify_ledger.py` checks the canonical results against the committed
outputs, and `pipeline/verify_sources.py` checks that every decimal value in
`manuscript/MANUSCRIPT.md` and `manuscript/SUPPLEMENT.md` is derivable from
`outputs/` or declared as carried forward. That gate matches decimal tokens
against the output tables; it is a provenance check on reported values, not a
claim-level audit. From the extraction files the pipeline reproduces all 33
